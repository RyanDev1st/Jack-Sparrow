...
export function useUserSessionState() {
  ...
  const [activeAssignedClueIds, setActiveAssignedClueIds] = useState<string[]>([]);
    ...
    try {
      ...
      if (raw) {
        try {
          ...
          setActiveAssignedClueIds(Array.isArray(session.activeAssignedClueIds) ? session.activeAssignedClueIds.filter(Boolean) : []);
  ...
  }, [activeAssignedClueIds, activeMantraCode, activeMantraId, activeMapSetId, activeSessionId, completedMapSetIds, hasStarted, hydrated, message, progress, scannedLocationCodes, status]);
