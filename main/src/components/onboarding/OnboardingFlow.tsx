...
import SplineOrb from "@/components/spline/SplineOrb";
import { Play, MapPin, QrCode, Puzzle, Trophy, ChevronRight } from "lucide-react";
import { FluidTextMorph } from "@/components/ui/FluidTextMorph";
import type { Application as SplineApplication, SplineEvent } from "@splinetool/runtime";
import { JOIN_KEYBOARD_SCENE_URL, preloadSplineScene } from "@/lib/spline-scenes";
...
const QRCheckinStage = lazy(() => import("@/components/onboarding/QRCheckinStage"));
const preloadScanRoute = () => import("@/pages/Scan");
const preloadScanStage = () => import("@/components/onboarding/QRCheckinStage");
...
export default function OnboardingFlow({ stage }: OnboardingFlowProps) {
  ...
  useEffect(() => {
    ...
    void preloadSplineScene(JOIN_KEYBOARD_SCENE_URL).catch(() => {});
  }, [stage]);
    ...
    const linkEl = container.querySelector<HTMLElement>("[data-shining-link]");
      ...
      if (container) {
        nodeEls.forEach((el) => {
          ...
          const baseAngle = (index / NODES.length) * 360 + 270;
        ...
        if (linkEl) {
          ...
          const startBaseAngle = (0 / NODES.length) * 360 + 270;
          const startAngle = (startBaseAngle + angleRef.current) % 360;
          const startRad = (startAngle * Math.PI) / 180;
          const startX = heroLayout.linkRadius * Math.cos(startRad);
          const startY = heroLayout.linkRadius * Math.sin(startRad) + Math.sin(time * 0.0016) * heroLayout.gravityAmplitude;
        ...
        {stage === "orbital" && (
          ...
          >
            ...
            {NODES.map((node, index) => {
              ...
              const nodePulseDuration = 3.8 + index * 0.35;
              ...
              return (
                ...
                >
                  ...
                  >
                    ...
                    {!isStart && (
                      <motion.div
                        animate={{ opacity: isHovered ? 0.62 : [0.18, 0.34, 0.18], scale: isHovered ? 1.12 : [0.96, 1.08, 0.96] }}
                        transition={{ duration: nodePulseDuration, repeat: Infinity, ease: "easeInOut" }}
                        style={{
                          position: "absolute",
                    ...
                    <motion.div
                      animate={{
                        y: isStart ? 0 : (isHovered ? -5 : [0, -3, 0]),
                        scale: isStart ? (isHovered ? 1.04 : 1) : (isHovered ? 1.06 : 1),
                      }}
                      transition={{ duration: isHovered ? 0.25 : nodePulseDuration, repeat: isHovered ? 0 : Infinity, ease: "easeInOut" }}
                      style={{
                        width: `${isStart ? heroLayout.startNodeSize : heroLayout.otherNodeSize}px`,
                        height: `${isStart ? heroLayout.startNodeSize : heroLayout.otherNodeSize}px`,
                        borderRadius: "50%",
                    ...
                    >
                      <motion.div
                        animate={{ scale: isHovered ? 1.08 : [1, 1.04, 1] }}
                        transition={{ duration: isHovered ? 0.2 : nodePulseDuration, repeat: isHovered ? 0 : Infinity, ease: "easeInOut" }}
