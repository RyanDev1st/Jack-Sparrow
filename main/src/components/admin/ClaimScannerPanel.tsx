...
export default function ClaimScannerPanel() {
  ...
  const [message, setMessage] = useState('Scan player completion QR (session_id).');
  ...
  return (
    <GlassCard className="p-5">
      ...
      <p className="mt-2 text-sm text-white/75">Scan QR or enter session/mantra code manually for fallback validation.</p>
      ...
      >
        <input
          ...
          placeholder="ENTER SESSION_ID / MANTRA_ID / MANTRA_CODE"
      ...
      <div className="mt-3 rounded-lg border border-white/12 bg-[#020611]/70 p-3">
        ...
        {result && (
          <div className="mt-2 space-y-1 text-xs text-white/75">
            ...
            <p>Mantra: {result.mantraId}</p>
            <p>Code: {result.mantraCode}</p>
            <p>Hunter: {result.hunterStatus}</p>
            {typeof result.totalUserClaims === 'number' && <p>User Total Completions: {result.totalUserClaims}</p>}
          </div>
        )}
        {result && (result.totalUserClaims ?? 0) > 2 && Array.isArray(result.repeatHistory) && result.repeatHistory.length > 0 && (
          <div className="mt-3 rounded-lg border border-orange-300/35 bg-[#2a1306]/45 p-3">
            <p className="text-xs font-semibold uppercase tracking-[0.14em] text-orange-200">Repeat User Audit Log (3+)</p>
