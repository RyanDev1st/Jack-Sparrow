      ...
      >
        {state === 'finished' && hunter && (
          <GlassCard className="border-2 border-orange-300 p-6 shadow-[0_0_32px_rgba(245,130,32,0.35)]">
            ...
            <p className="mt-5 text-sm text-white/70">Status check only. Use the Claim Scanner panel to finalize claim.</p>
          </GlassCard>
        )}
        {state === 'hunting' && hunter && (
          <GlassCard className="p-6">
            <p className="text-2xl font-bold text-white/90">STILL HUNTING</p>
            <p className="mt-2 text-base text-white/75">Missing {missingCount} nodes.</p>
          </GlassCard>
        )}
        {state === 'claimed' && hunter && (
          <GlassCard className="p-6">
            <p className="text-2xl font-bold text-orange-200">ALREADY CLAIMED</p>
            <p className="mt-2 text-base text-white/75">This mantra has already been finalized.</p>
          </GlassCard>
        )}
        ...
        {claimAudit && claimAudit.ok && (
          <GlassCard className="mt-4 border border-orange-300/40 p-5">
            <p className="text-xs uppercase tracking-[0.2em] text-orange-200">Claim Check-in Audit</p>
            <p className="mt-2 text-sm text-orange-100">Logged attempts: {claimAudit.totalUserClaims}</p>
            {claimAudit.repeatHistory.length > 0 ? (
              ...
              <p className="mt-3 text-xs text-white/70">No claim audit history recorded yet for this mantra/session.</p>
            )}
          </GlassCard>
        )}
      </motion.div>
